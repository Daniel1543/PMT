...
function pmt () {
    ...
    if (valor !== '' && i !== '' && n !== '' && renda !== ''  && entrada !== '') 
    {
        ...
        if (prestacao > final)
        {
            resultado.textContent = `O valor da prestação que é ${prestacao} não pode ultrapassar 30% (${final}) da sua renda, que é ${renda}`
        }
        else
        {
            resultado.textContent = `Compra realizada! O valor da prestação será de ${prestacao}`
        }
